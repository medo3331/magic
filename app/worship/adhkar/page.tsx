      ...
      <main className="flex-1 px-4 py-6 md:px-6 md:py-8">
        <div className="mx-auto flex max-w-5xl flex-col gap-6">
          ...
          <Reveal index={0}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[#9AA0C0] mt-1">أذكار الصباح والمساء والنوم وبعد الصلاة</p>
              </div>
            ...
            <Reveal index={1}>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {categories.map((cat, index) => {
                  const adhkar = mockAdhkar[cat.id as keyof typeof mockAdhkar];
                  const progress = mockAdhkarProgress[cat.id as keyof typeof mockAdhkarProgress];
                  const totalDhikrs = adhkar.length;
                  const totalCounts = adhkar.reduce((sum: number, d: typeof adhkar[0]) => sum + d.repeatCount, 0);
                  const completedCounts = progress?.currentCount || 0;
                  const progressPct = totalCounts > 0 ? (completedCounts / totalCounts) * 100 : 0;
                  ...
                  return (
                    <AdhkarCategoryCard
                      ...
                      icon={CategoryIcon}
                      color={cat.color}
                      bg={cat.color.replace("text-", "bg-").replace("]", "/15]")}
          ...
          {showReader && selectedCategory && (
            <Reveal index={1}>
              <AdhkarReader
                adhkar={mockAdhkar[selectedCategory as keyof typeof mockAdhkar]}
            ...
            <Reveal index={2}>
              <GlassCard className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-white mb-2">عن الأذكار</h3>
                    <p className="text-[#C7CBE6] leading-relaxed">
                      الأذكار هي كلماتٌ يذُكر بها العبدُ ربه سبحانه وتعالى في أوقاتٍ مخصوصة، 
                      وقد جاءت بها السنّة النبوية المطهرة. المحافظة عليها سببٌ للطمأنينة 
                      والبركة في الوقت والعمل، وحفظٌ من كل سوء.
                    </p>
